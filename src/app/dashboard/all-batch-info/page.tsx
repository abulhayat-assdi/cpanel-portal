...
export default function AllBatchInfoPage() {
    ...
    // Grid Data Ref
    const MAX_ROWS = 200;
    const COLUMNS = ["roll", "name", "phone", "address", "courseStatus", "currentlyDoing", "companyName", "businessName", "salary"];
    ...
    // Derive stats from filtered batch (or all when 'all')
    const filteredStudents = selectedSearchBatch === "all" ? allStudents : allStudents.filter(s => s.batchName === selectedSearchBatch);

    // Derived Statistics (always based on filteredStudents for real-time batch filtering)
    const uniqueBatchesCount = new Set(allStudents.map(s => s.batchName)).size;
    ...
    const overviewStats = [
        ...
        { label: "Total Earning / Month", value: formattedEarnings, icon: CurrencyDollarIcon, color: "bg-emerald-600", lightColor: "bg-emerald-100", textColor: "text-emerald-800" },
    ];
    ...
    const handleSaveBatchInfo = async () => {
        ...
        try {
            ...
            await saveBatchInfo(newBatchName, cleanData);
            await fetchData();
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[#6b7280] mt-1">
                            Comprehensive overview of batch statistics, student outcomes, and total earnings.
                        </p>
                    </div>
                </div>
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3 text-sm border-b border-gray-200">
                                                <span className={`px-2 py-1 rounded text-xs font-semibold ${
                                                    student.courseStatus === 'Completed' ? 'bg-emerald-100/50 text-emerald-700' :
                                            ...
                                            <td className="px-4 py-3 text-sm border-b border-gray-200">
                                                <span className={`px-2 py-1 rounded text-xs font-semibold ${
                                                    student.currentlyDoing === 'Job' ? 'bg-cyan-100/50 text-cyan-700' :
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[80vh] animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6 overflow-y-auto flex-1">
                            <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                                ...
                                {Array.from(new Set(allStudents.map(s => s.batchName))).map((bName) => (
                                    <button
                                        key={bName}
                                    ...
                                    >
                                        ...
                                        <div className="text-center px-2">
                                            <span className="font-semibold text-gray-800 text-sm block truncate w-full">{bName}</span>
                                            <span className="text-xs text-gray-500 mt-0.5">{allStudents.filter(s => s.batchName === bName).length} Students</span>
                                        </div>
                                    </button>
                                ))}
                            </div>
